...
为什么要有它：JS 测试断言的是「属性 / class」，而真正决定用户看到什么的是 CSS。
...
check('HTML 上不带 hidden 属性（避免 !important 与 .show 打架）',
      'id="rail-drop-hint"hidden' not in flat_html and 'id="rail-drop-hint"' in flat_html)
check('JS 用 class 切换而非 hidden 属性',
      'hint.classList.add(\'show\')' in re.sub(r'\s+', '', app).replace('"', "'")
      and 'hint.hidden' not in app)
check('拖拽态样式仍在', '.compose-rail.drag-over{' in flat_css)
check('dragleave 不判断文件类型（types 可能为空）',
      "rail.addEventListener('dragleave'" in app and 'if(!active)return' in re.sub(r'\s+', '', app))
...
check('账号区已不在左侧胶囊内',
      html.index('class="sidebar"') < html.index('class="corner-actions"'))
check('旧的 side-foot 已彻底移除', 'side-foot' not in css and 'side-foot' not in html)
...
print()
print('=== [G] 特效设置与交互 ===')
fjs = re.sub(r'\s+', '', (ROOT / 'assets' / 'js' / 'settings.js').read_text(encoding='utf-8'))
check('浅色 --surface-3 有真值', '--surface-3:#eceef6' in flat_css)
check('浅色 --line-hover 有真值', '--line-hover:#c8cde3' in flat_css)
check('特效卡片带预览带（fx-demo）', "class=\"fx-demo\"" in re.sub(r'\s+', ' ', (ROOT / 'index.html').read_text(encoding='utf-8')) or 'fx-demo' in fjs)
...
check('滑杆禁用逻辑存在', 'data-set="intensity"' in fjs and 'el.disabled=fxOff' in fjs)
check('aria-pressed 随状态同步', "setAttribute('aria-pressed'" in fjs)
...
print()
print('=== [H] 自定义 JS 动效 ===')
fxjs = re.sub(r'\s+', '', (ROOT / 'assets' / 'js' / 'effects.js').read_text(encoding='utf-8'))
...
check('示例用到 fx.ctx / fx.state', 'fx.ctx' in (ROOT / 'assets' / 'js' / 'settings.js').read_text(encoding='utf-8'))
...
check('收件箱两组都挂了指示器', app.count('class="ink-group" data-ink=') >= 2)
...
check('重复点同一项直接返回', "===el.dataset.kind)return" in flat_app and "===el.dataset.scope)return" in flat_app)
check('列表切换淡入', '.list.is-swapping{animation:listSwap' in flat_css)
check('窗口尺寸变化时重排（不动画）', "addEventListener('resize',()=>scheduleFilterInk(false))" in flat_app)

print()
print('=== [J] 悬浮方块的拖动与磁吸 ===')
html = (ROOT / 'index.html').read_text(encoding='utf-8')
check('两栏都有拖动手柄', 'data-grip="sidebar"' in html and 'data-grip="rail"' in html)
check('手柄是 button（键盘可达）', html.count('<button type="button" class="float-grip"') == 2)
...
check('三档吸附位', 'function floatSlots(' in app)
check('位置钳制在视口内', 'function clampTop(' in app)
...
print()
print('=== [K] 点击即时反馈与内容居中 ===')
check('点击后先动指示器再等数据', 'applyFilterSelection()' in flat_app)
check('内容区左右让位对称', re.search(
...
check('键盘用法有可见提示', 'class="side-hint"' in html and '↑↓' in html and '←→' in html)
check('上下切视图', 'function cycleView(' in app)
check('左右切栏目', 'function cycleFilter(' in app)
check('顺序从 DOM 读，不另维护常量', "querySelectorAll('#tabs [data-view]')" in app)
check('键盘走程序化点击（不另写一套切换逻辑）', app.count('next.click()') >= 2)
...
check('未登录时不响应（别在看不见的地方改视图）',
      "$('#app-view').classList.contains('hidden'))return" in flat_app)
check('浮层打开时让路（模态 / 设置 / 抽屉）',
      "querySelector('.modal-mask'))return" in flat_app
      and 'classList.contains(\'nav-open\'))return' in flat_app
      and 'classList.contains(\'rail-open\'))return' in flat_app)
...
check('模式切换同步 aria-selected', "setAttribute('aria-selected',String(on))" in sjs)
...
check('密钥字段用 password 类型', 'type="password"class="set-input"data-vendor-field="apiKey"' in sjs)
...
check('按钮里不再嵌套按钮（点选与删除是兄弟）',
      sjs.count('class="model-pick"') == 1 and sjs.count('class="model-del"') == 1)
...
check('取消请求不被当成失败吞掉', "err.name==='AbortError'" in ajs)
check('生成参数只在有效时才带上',
