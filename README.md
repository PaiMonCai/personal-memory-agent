...
**自定义接口**。填接口地址、API Key、模型名，请求直连你自己的服务（OpenAI 兼容格式）。
地址要能拼成 `<base>/chat/completions`，尾斜杠会自动去掉；非 http(s) 会被拒。
面板会实时告诉你还缺什么（地址 → 密钥 → 模型名，顺序与字段顺序一致）。
...
但**本地缓存里刻意剔掉了 API Key** —— 那份缓存只是为了登录页不闪默认配色，没必要把密钥再往浏览器里存一份。
...
#   p e r s o n a l - m e m o r y - a g e n t  
 #   p e r s o n a l - m e m o r y - a g e n t  
 